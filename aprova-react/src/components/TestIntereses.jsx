...
import itemsIntereses from '../data/items_intereses.json'
...
function TestIntereses({ acceso, onVolver, onCompletado }) {
  ...
  const totalItems = itemsIntereses.items.length
  const totalPaginas = Math.ceil(totalItems / ITEMS_PER_PAGE)
  const itemsPagina = itemsIntereses.items.slice(pagina * ITEMS_PER_PAGE, (pagina + 1) * ITEMS_PER_PAGE)
  ...
  const calcularResultados = () => {
    const resultados = {}
    itemsIntereses.escalas.forEach(esc => {
      resultados[esc] = { puntaje: 0, maximo: 50, items: 0 }
    })

    itemsIntereses.items.forEach(item => {
      const resp = respuestas[item.id]
      if (resp && resultados[item.escala]) {
        resultados[item.escala].puntaje += resp
        resultados[item.escala].items++
      }
    })

    Object.keys(resultados).forEach(esc => {
      const r = resultados[esc]
    ...
    try {
      const response = await fetch(`${API_URL}/api/enviar-resultados`, {
        ...
        body: JSON.stringify({
          ...
          testNombre: 'BRP - Inventario de Intereses Ocupacionales',
          respuestas: resultados,
          modalidad: acceso.modalidad,
          tipoResultado: 'intereses'
        })
      })

      const data = await response.json()
      if (data.success) {
        // Guardar top 3 intereses en localStorage para diagnóstico diferencial
    ...
    return (
      <div className="test-aptitudes" ref={topRef}>
        <div style={{
          ...
          <h2 style={{ color: '#26215C', margin: '0 0 8px' }}>Test de Intereses Ocupacionales BRP</h2>
          <p style={{ color: '#6b7280', fontSize: '15px', lineHeight: '1.6', margin: '0 0 20px' }}>
            Este test contiene <strong>130 actividades</strong> y tiene una duración aproximada de <strong>30-45 minutos</strong>.
          </p>
          <div style={{
    ...
    <div className="test-aptitudes" ref={topRef}>
      <div className="aptitudes-header">
        ...
        <div className="aptitudes-info">
          <span className="aptitudes-titulo">Intereses</span>
      ...
      <div className="aptitudes-instrucciones">
        <p>Califica cada actividad según tu gusto: <strong>5</strong> = Me gusta mucho &nbsp; <strong>4</strong> = Me gusta &nbsp; <strong>3</strong> = Me es indiferente &nbsp; <strong>2</strong> = Me desagrada &nbsp; <strong>1</strong> = Me desagrada mucho</p>
      </div>
          ...
          return (
            <div key={item.id} className={`aptitudes-item ${seleccionada ? 'respondida' : ''}`}>
              ...
              <div className="item-opciones">
                {itemsIntereses.opciones.map((opcion) => (
                  <button
                    key={opcion.valor}
